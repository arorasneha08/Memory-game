(function () {
    ...
    if (!user) { window.location.href = "../pages/register.html"; return; }
    ...
    function showNotif(text, timeout) { if (!timeout) timeout = 1600; notif.textContent = text; notif.classList.remove('hidden'); setTimeout(function () { notif.classList.add('hidden'); }, timeout); }
    ...
    function buildDeck() {
        ...
        if (selectedTheme == 'emoji') {
            ...
            else { for (var k in emojiPools) { pool = pool.concat(emojiPools[k]); } }
            ...
            else { for (var k in triviaPools) { pool = pool.concat(triviaPools[k]); } }
            pool = shuffle(pool).slice(0, lvl.pairs);
            for (var i = 0; i < pool.length; i++) {
    ...
    function clickCard(card) {
        if (state.busy || state.flipped.indexOf(card) >= 0 || card.classList.contains('matched') || Date.now() < state.frozenUntil) { showNotif("Board frozen!"); return; }
    ...
    function checkMatch() {
        ...
        var ca = state.cards[a.dataset.index], cb = state.cards[b.dataset.index];
        var match = ca.pair == cb.pair && (ca.type !== 'trivia' || ca.side != cb.side);
        ...
        if (match) {
            ...
            setTimeout(() => {
                ...
                state.flipped = []; state.busy = false;
                if (selectedMode === 'multi') { state.currentPlayer = (state.currentPlayer === 1 ? 2 : 1); updateTurnUI(); }
            }, 700);
        }
    }
    ...
    function startTimer() {
        ...
        state.timerId = setInterval(() => {
            ...
            if (state.timeLeft <= 0) { clearInterval(state.timerId); endGame(false); }
        }, 1000);
    }
    ...
    function endGame(win) {
        ...
        var title = '', text = '', status = '';
        if (selectedMode === 'multi') {
            var p1 = state.scores[1] || 0, p2 = state.scores[2] || 0;
            if (!win && state.timeLeft <= 0 && state.matchedCount < state.totalPairs) {
                if (p1 === p2) { title = "Time's Up — Tie!"; text = `Both tied ${p1}`; status = 'Tie'; }
                else if (p1 > p2) { title = "Player 1 Wins!"; text = `P1 ${p1} — P2 ${p2}`; status = 'Win'; }
                else { title = "Player 2 Wins!"; text = `P2 ${p2} — P1 ${p1}`; status = 'Win'; }
            } else {
                if (p1 === p2) { title = "Tie!"; text = `Both ${p1}`; status = 'Tie'; }
                else if (p1 > p2) { title = "Congrats P1!"; text = `P1 ${p1} — P2 ${p2}`; status = 'Win'; }
                else { title = "Congrats P2!"; text = `P2 ${p2} — P1 ${p1}`; status = 'Win'; }
            }
        } else {
            if (win) { title = "Congrats!"; text = `All pairs matched ${state.moves.single} moves, ${state.scores[1]} pts`; status = 'Win'; }
            else { title = "Game Over"; text = `Time up, matched ${state.matchedCount}/${state.totalPairs}`; status = 'Lose'; }
        }
        ...
        gameHistory.push({
            ...
            player: user.name,
            moves: selectedMode === 'multi' ? `${state.moves[1]} / ${state.moves[2]}` : state.moves.single,
            time: formatTime(levelSettings[selectedDifficulty].time - state.timeLeft),
    ...
    // --- Powerups ---
    function revealAll(temp) { if (!temp) temp = 2500; if (state.busy) return; state.busy = true; const cards = board.getElementsByClassName('card'); for (let i = 0; i < cards.length; i++) if (!cards[i].classList.contains('matched')) cards[i].classList.add('flipped'); setTimeout(() => { for (let i = 0; i < cards.length; i++) cards[i].classList.remove('flipped'); state.busy = false; }, temp); }
    function freezeOpponent(dur) { if (!dur) dur = 6000; state.frozenUntil = Date.now() + dur; showNotif("Board frozen for " + Math.ceil(dur / 1000) + "s"); }

    if (p1RevealBtn) p1RevealBtn.onclick = () => { if (!state.usedPowerup.p1Reveal) { revealAll(3000); state.usedPowerup.p1Reveal = true; p1RevealBtn.disabled = true; } };
    if (p2RevealBtn) p2RevealBtn.onclick = () => { if (!state.usedPowerup.p2Reveal) { revealAll(3000); state.usedPowerup.p2Reveal = true; p2RevealBtn.disabled = true; } };
    if (p1FreezeBtn) p1FreezeBtn.onclick = () => { if (!state.usedPowerup.p1Freeze) { freezeOpponent(6000); state.usedPowerup.p1Freeze = true; p1FreezeBtn.disabled = true; } };
    if (p2FreezeBtn) p2FreezeBtn.onclick = () => { if (!state.usedPowerup.p2Freeze) { freezeOpponent(6000); state.usedPowerup.p2Freeze = true; p2FreezeBtn.disabled = true; } };
    ...
    function initGame() {
        ...
        buildDeck(); renderBoard(); updateScoreAndMovesUI(); updateTurnUI(); startTimer();
    ...
    playAgainBtn.onclick = () => { resultModal.classList.remove('show'); resultModal.classList.add('hidden'); initGame(); };
    dashboardBtn.onclick = () => { window.location.href = "../pages/dashboard.html"; };
